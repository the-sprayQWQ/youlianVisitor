...
const visitorList = computed(() => {
  if (activeTab.value === 0) {
    // 待审核
    return allVisitorList.value.filter(item => item.status === 'waiting')
  }
  else {
    // 在厂访客
    return allVisitorList.value.filter(item => item.status === 'approved')
  }
})
