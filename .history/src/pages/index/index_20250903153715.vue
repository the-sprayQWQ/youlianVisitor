...
function handleRecordClick(id: number, roleType: string, applyId?: string) {
    ...
    <scroll-view class="main-content" scroll-y :show-scrollbar="false">
      ...
      <view class="recent-application">
        ...
        <view class="record-list">
          <!-- 记录项1 -->
          <view v-for="(item, index) in applyList" :key="item.id" class="record-item" @click="handleRecordClick(item.id, roleType, item.applicationCode)">
            <view class="record-content">
              <text class="record-title">业务沟通</text>
              <text class="record-info">申请单号：{{ item.applicationCode }}</text>
              <view class="record-details">
                <text class="record-detail">当前节点：{{ item.currentName }}</text>
                <text class="record-detail">审批人：李晓明</text>
              </view>
              <text class="record-time">{{ item.createBy }}</text>
            </view>
