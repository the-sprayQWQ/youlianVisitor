  ...
  try {
    ...
    if (loginType === 'account') {
      ...
      if (result) {
        ...
        console.log('当前角色类型:', userStore.roleType)
        ...
        else if (userStore.roleType === '部门审批' || userStore.roleType === '领导审批') {
      ...
      // 手机号登录表单验证
      if (!phoneFormData.value.phone.trim()) {
        uni.showToast({
          title: '请输入手机号',
          icon: 'none',
        })
      ...
      if (!phoneFormData.value.password.trim()) {
        uni.showToast({
          title: '请输入密码',
          icon: 'none',
        })
      ...
      const result = await userStore.phoneLogin(phoneFormData.value)
