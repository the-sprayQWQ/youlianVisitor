...
function handleView(record: any) {
  ...
  // 这里可以跳转到详情页面
  uni.navigateTo({
    url: `/pages/visitorApplication/visitorApplication?applyId=${record.applicationCode}&applicationType=${record.applicationName}&roleType=${roleType.value}&view=${views}&currentNode=${currentNode}`,
  })
}
  ...
  <view class="audit-page">
    ...
    <view class="records-list">
      ...
      <view v-show="activeTab === 'pending'" class="records-container">
        ...
        <!-- 没有更多数据 -->
        <view
          v-else-if="!pendingHasMore && pendingRecords.length > 0"
          class="no-more"
        >
        ...
        <!-- 暂无数据 -->
        <view
          v-if="pendingRecords.length === 0 && !pendingIsLoading"
          class="empty-state"
        >
      ...
      <view v-show="activeTab === 'approved'" class="records-container">
        ...
        <!-- 没有更多数据 -->
        <view
          v-else-if="!approvedHasMore && approvedRecords.length > 0"
          class="no-more"
        >
          <text class="no-more-text">没有更多数据了</text>
        </view>
        ...
        <!-- 暂无数据 -->
        <view
          v-if="approvedRecords.length === 0 && !approvedIsLoading"
          class="empty-state"
        >
          <view class="empty-icon">
            <view class="i-carbon-document-blank empty-icon-img" />
