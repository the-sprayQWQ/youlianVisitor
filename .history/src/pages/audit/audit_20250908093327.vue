...
onLoad(async () => {
  await getProcessList('0', LoginUserId)
})

async function getProcessList(status: string, currentLoginedUser: string) {
  const res = await getYlfkProcessApplication(status, currentLoginedUser)
  if (res.code === 200) {
...
async function switchTab(tab: string) {
  ...
  if (tab === 'pending') {
    ...
    getProcessList('1', LoginUserId)
  }
}

async function toApprove(applicationCode: string, roleType: string, applicationType: string) {
...
function handleView(record: any) {
  ...
  // 这里可以跳转到详情页面
  uni.navigateTo({
    url: `/pages/visitorApplication/visitorApplication?applyId=${record.applicationCode}&applicationType=${record.applicationName}&roleType='访客'`,
  })
}
    ...
    <view class="records-list">
      ...
      <view v-show="activeTab === 'pending'" class="records-container">
        <view
          ...
          :key="record.id"
          class="record-item"
          @click="toApprove(record.applicationCode, roleType, record.applicationName)"
        >
      ...
      <view v-show="activeTab === 'approved'" class="records-container">
        ...
        >
          <view class="item-header">
            <text class="item-id">申请单号: {{ record.applicationCode }}</text>
            <view class="status-tag" :class="getStatusClass(record.applicationStatus)">
