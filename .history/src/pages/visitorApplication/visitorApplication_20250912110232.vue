...
onLoad(async (e) => {
  ...
  if (e.roleType === '访客') {
    ...
    const temp = opinionsList.value.filter(
      item => item.applicatiOpinion === '同意入厂',
    )
    const temp2 = opinionsList.value.filter(item => item.applicatiOpinion === '已阅')
    if (temp[0].applicatiOpinion === '同意入厂') {
      approvals.value.manager = true
    }
    if (temp2[0].applicatiOpinion === '已阅') {
