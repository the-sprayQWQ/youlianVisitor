...
onLoad(async (e) => {
  await getAllUser()
  console.log('onLoad params:', e) // 添加调试日志
  if (e.applyId && e.applyId !== 'undefined') {
    ...
    if (e.applicationType === '0' && e.currentNode === '入厂门卫二维码检验') {
      ...
      console.log('门卫检验节点 - opinionsList:', opinionsList.value) // 添加调试日志
      ...
      await getResult(e.applyId)
      console.log('部门审核节点 - opinionsList:', opinionsList.value) // 添加调试日志
      isApproval.value = true
      managerApp.value = true
      // 检查部门审核结果
  ...
  console.log('最终状态:', {
    ...
    managerApp: managerApp.value,
    doorApp: doorApp.value,
...
<template>
  <view class="container">
    ...
    <!-- 审批框 - 添加调试信息 -->
    <view v-if="applyId && isApproval" class="mb-12 box-border p-3" style="border: 2px solid red;">
      <text style="color: red; font-size: 12px;">
        调试: applyId={{ applyId }}, isApproval={{ isApproval }}, managerApp={{ managerApp }}, doorApp={{ doorApp }}
      </text>
      <text style="color: blue; font-size: 12px; display: block;">
        managerRejected={{ approvalStatus.managerRejected }}, guardRejected={{ approvalStatus.guardRejected }}
      </text>
