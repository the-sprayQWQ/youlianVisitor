...
// 添加状态管理方法
function getApprovalStatusClass(type) {
  const status = approvalStatus.value[type]
  if (status === 'approved')
...
function getStatusTextClass(type) {
  const status = approvalStatus.value[type]
...
onLoad(async (e) => {
  ...
  if (e.applyId && e.applyId !== 'undefined') {
    ...
    if (e.applicationType === '0' && e.currentNode === '入厂门卫二维码检验') {
      ...
      console.log(opinionsList.value[1], '部门审核')
      if (opinionsList.value[1].applicatiOpinion === '同意入厂') {
        approvals.value.manager = true
        approvalStatus.value.manager = 'approved'
      }
      else {
        approvals.value.manager = false
        approvalStatus.value.manager = 'rejected'
      }
      if (opinionsList.value[2].applicatiOpinion === '已阅') {
        approvals.value.guard = true
        approvalStatus.value.guard = 'approved'
      }
      else {
        approvals.value.guard = false
        approvalStatus.value.guard = 'rejected'
      }
  ...
  if (e.roleType === '访客') {
    ...
    if (e.view === 'true') {
      ...
      if (tempArr[0].applicatiOpinion === '同意入厂') {
        approvals.value.manager = true
        approvalStatus.value.manager = 'approved'
      }
      else {
    ...
    <view v-if="applyId && isApproval" class="mb-12 box-border p-3">
      <view class="approval-section">
        <view v-if="managerApp" class="approval-item">
          ...
          <label class="approval-checkbox">
            <wd-checkbox
              ...
              :class="getApprovalStatusClass('manager')"
              :disabled="true"
            >
              同意入厂
              <text :class="getStatusTextClass('manager')">
        ...
        <view v-if="divisionApp" class="approval-item">
          ...
          <label class="approval-checkbox">
            <wd-checkbox
              v-model="approvals.leader"
              :class="getApprovalStatusClass('leader')"
              :disabled="true"
            >
              同意入厂
              <text :class="getStatusTextClass('leader')">
          ...
          <view style="display: flex; justify-content: space-between; align-items: center">
            ...
            <label class="approval-checkbox">
              <wd-checkbox
                ...
                :class="getApprovalStatusClass('guard')"
                :disabled="true"
              >
                已阅
                <text :class="getStatusTextClass('guard')">
                  {{ getStatusText('guard') }}
                </text>
              </wd-checkbox>
            </label>
...
// 审批状态样式
.wd-checkbox.approval-status-approved .wd-checkbox__icon-wrap.is-checked {
  background-color: #07c160 !important;
  border-color: #07c160 !important;
}

.wd-checkbox.approval-status-rejected .wd-checkbox__icon-wrap.is-checked {
  background-color: #ee0a24 !important;
  border-color: #ee0a24 !important;
}

.wd-checkbox.approval-status-rejected .wd-checkbox__icon-wrap.is-checked .wd-checkbox__icon {
  display: none !important;
}

.wd-checkbox.approval-status-rejected .wd-checkbox__icon-wrap.is-checked::before,
.wd-checkbox.approval-status-rejected .wd-checkbox__icon-wrap.is-checked::after {
  content: '';
  position: absolute;
  left: 50%;
  top: 50%;
