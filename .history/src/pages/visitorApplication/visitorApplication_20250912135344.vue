...
onLoad(async (e) => {
  ...
  if (e.applyId && e.applyId !== 'undefined') {
    ...
    if (res.code === 200) {
      ...
      form1.carNumber
        = res.result.ylfkApplicationPage.carNumber
      form1.visitorNumber
        = res.result.ylfkApplicationPage.visitorNumber
  ...
  if (e.roleType === '访客') {
    ...
    const temp = opinionsList.value.filter(item => item.nodeName === e.currentNode)
    if (e.currentNode === '入厂门卫二维码检验') {
      approvals.value.guard = true
      approvals.value.leader = true
      approvals.value.manager = true
    }
    else if ((e.currentNode === '部门审核' || e.currentNode === '部门经理审核') && temp[0].applicatiResult_dictText === '通过') {
      approvals.value.manager = true
    }
    else if ((e.currentNode === '部门审核' || e.currentNode === '部门经理审核') && temp[0].applicatiResult_dictText !== '通过') {
    ...
    else if (e.currentNode === '门卫审核' && temp[0].applicatiResult_dictText === '通过') {
      approvals.value.manager = true
      approvals.value.guard = true
    }
    else if (e.currentNode === '门卫审核' && temp[0].applicatiResult_dictText !== '通过') {
    ...
    else if (e.currentNode === '分管领导审核' && temp[0].applicatiResult_dictText === '通过') {
      approvals.value.leader = true
      approvals.value.manager = true
      approvals.value.guard = false
      // if (temp[0].applicatiResult_dictText === '驳回') {
      //   approvalStatus.value.leaderRejected = true
      // }
    }
  }
  ...
  <view class="container">
    ...
    <view v-if="applyId" class="mb-12 box-border p-3">
      <view class="approval-section">
        <view
          v-if="['入厂门卫二维码检验', '部门审核'].includes(currentNode)"
          class="approval-item"
        >
          ...
          <label class="approval-checkbox">
            ...
            <!-- 新增：红色×复选框 -->
            <view v-else class="approval-checkbox-rejected">
        ...
        >
          ...
          <label class="approval-checkbox">
            <wd-checkbox v-model="approvals.leader" :disabled="isView"> 同意入厂 </wd-checkbox>
          </label>
        </view>
