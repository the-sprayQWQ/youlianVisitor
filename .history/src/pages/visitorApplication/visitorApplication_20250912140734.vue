...
onLoad(async (e) => {
  ...
  if (e.currentNode === '入厂门卫二维码检验') {
    ...
    (e.currentNode === '部门审核' || e.currentNode === '部门经理审核')
  ...
  if (e.applyId && e.applyId !== 'undefined') {
    ...
    if (res.code === 200) {
      ...
      form1.carNumber = res.result.ylfkApplicationPage.carNumber
      form1.visitorNumber = res.result.ylfkApplicationPage.visitorNumber
    ...
    <view v-if="applyId" class="mb-12 box-border p-3">
      <view class="approval-section">
        ...
        >
          ...
          <label class="approval-checkbox">
            <wd-checkbox v-model="approvals.leader" :disabled="isView">
              同意入厂
            </wd-checkbox>
          </label>
        </view>
