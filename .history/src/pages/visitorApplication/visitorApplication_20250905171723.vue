  ...
  if (e.applyId && e.applyId !== 'undefined') {
    ...
    if (res.code === 200) {
      ...
      if (form.value.applicationType === '0') {
        console.log(form.value.applicationType, 'form.value.applicationType')
...
async function Approval(user: string, applicationCode: string, applicationType: string, transUser?: string, copyUser?: string, userOpinion?: string, applicationResult?: string) {
  const res = await toApprovalApplication(user, applicationCode, applicationType, transUser, copyUser, userOpinion, applicationResult)
}
</script>
