...
import { getTime } from '@alova/shared'
...
const approvals = reactive<visitor.Approvals>({
  manager: false,
  leader: false,
  guard: false,
})
...
async function onVisitorTypeChange(e: any) {
  ...
  form.value.applicationType = visitorTypes.value[e.detail.value].value
...
function onApprovalChange(type: keyof visitor.Approvals, e: any) {
  approvals[type] = e.detail.value
}
...
onLoad(async (e) => {
  if (e.applyId && e.applyId !== 'undefined') {
    ...
    const res = await getYlfkApplicationDetail(e.applyId)
    if (res.code === 200) {
      form.value = res.result
      timeStampList.value = [getTimeStamp(form.value.startTime), getTimeStamp(form.value.endTime)]
  ...
  <view class="container">
    ...
    <scroll-view class="content" scroll-y>
      <study-form
        ...
        :value="timeStampList"
      />
      <business-form v-show="selectedVisitorType.value === '0'" ref="businessFormRef" :disabled="disable" :apply-id="applyId" :form="form" :value="timeStampList" />
    </scroll-view>
    ...
    <view v-if="applyId && isShow" class="mb-12 box-border p-3">
      <view class="approval-section">
        <view class="approval-item">
          ...
          <label class="approval-checkbox">
            <checkbox
              ...
              @change="onApprovalChange('manager', $event)"
            />
        ...
        <view class="approval-item">
          ...
          <label class="approval-checkbox">
            <checkbox
              :checked="approvals.leader"
              @change="onApprovalChange('leader', $event)"
            />
            <text class="approval-text">同意入厂</text>
          </label>
        </view>
        <view class="approval-item-extra">
          <view style="display: flex; justify-content: space-between">
            <text class="approval-title">门卫审批</text>
            <label class="approval-checkbox">
              <checkbox
                :checked="approvals.guard"
                @change="onApprovalChange('guard', $event)"
              />
              <text class="approval-text">已阅</text>
            </label>
          </view>
