...
onLoad(async (e) => {
  ...
  if (e.applyId) {
    ...
    if (res.code === 200) {
      ...
      // 如果来自已审批页面，获取审批历史
      if (isFromApproved.value && res.result && typeof res.result === 'object' && 'approvalHistory' in res.result) {
...
const shouldDisableCheckbox = computed(() => (nodeName) => {
  ...
  const notCurrentApprover = !isFromApproved.value && currentNode.value !== nodeName
...
function isNodeApproved(nodeName) {
  ...
  // 如果不是来自已审批页面，则所有节点都未审批
  if (!isFromApproved.value) {
    return false
  }

  // 检查审批历史中是否存在该节点的已审批记录
  const isApproved = approvalHistory.value.some(history =>
    history.nodeName === nodeName && history.status === 'approved',
  )
    ...
    >
      <form>
        ...
        <view v-if="applyId" class="form-section">
          ...
          <!-- 部门经理审核 -->
          <view
            v-if="shouldShowApprovalNode(['人资部行政部审核', '归口管理部门经理室审核'])"
            class="approval-card"
            :class="{ 'approved-node': isNodeApproved('归口管理部门经理室审核') }"
          >
            <text class="approval-title">
              归口管理部门经理室审核
              <text v-if="isNodeApproved('归口管理部门经理室审核')" class="approval-status">（已审批）</text>
            </text>
          ...
          <!-- 人资部审核 -->
          <view
            v-if="shouldShowApprovalNode(['门卫入厂审批', '人资部行政部审核']) && (isFromApproved || currentDepartment === '人资部')"
            class="approval-card"
            :class="{ 'approved-node': isNodeApproved('人资部行政部审核') }"
          >
            <text class="approval-title">
              人资部审核
              <text v-if="isNodeApproved('人资部行政部审核')" class="approval-status">（已审批）</text>
            </text>
            <label class="approval-checkbox">
              <wd-checkbox
          ...
          <!-- 行政部审核 -->
          <view
            v-if="shouldShowApprovalNode(['门卫入厂审批', '人资部行政部审核']) && (isFromApproved || currentDepartment === '行政部')"
            class="approval-card"
            :class="{ 'approved-node': isNodeApproved('人资部行政部审核') }"
          >
            <text class="approval-title">
          ...
          >
            <text class="approval-title">
              门卫入厂审核
              <text v-if="isNodeApproved('门卫入厂审批')" class="approval-status">（已审批）</text>
            </text>
            ...
            <view class="guard-section">
              ...
              <view class="info-row">
                <text class="info-label">车牌号：{{ form.ylfkCheckApplicationPage.carCode }}</text>
          ...
          >
            <text class="approval-title">
              门卫离厂审核
              <text v-if="isNodeApproved('门卫离厂审批')" class="approval-status">（已审批）</text>
            </text>
