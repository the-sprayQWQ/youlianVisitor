...
// 人员类型监听器
watch(() => form.value?.ylfkCheckApplicationPage?.userType, (newValue) => {
  if (newValue && employeeTypes.value.length > 0) {
    const matchedType = employeeTypes.value.find(type => type.value === newValue)
    if (matchedType) {
...
// 所属部门监听器
watch(() => form.value?.ylfkCheckApplicationPage?.belongDept, (newValue) => {
...
const displayDepartmentName = computed(() => {
  ...
  // 如果没有，尝试从部门列表中实时查找
  if (form.value?.ylfkCheckApplicationPage?.belongDept && departments.value.length > 0) {
    const dept = departments.value.find(d => d.value === form.value.ylfkCheckApplicationPage.belongDept)
    if (dept) {
      // 找到后也更新 department
    ...
    <scroll-view class="content" scroll-y>
      <form>
        ...
        <view v-if="applyId" class="form-section">
          ...
          <!-- 部门经理审核 -->
          <view
            v-if="['归口管理部门经理室审核'].includes(currentNode)"
            class="approval-card"
          >
          ...
          <!-- 人资部审核 -->
          <view
            v-if="['人资部行政部审核'].includes(currentNode)"
            class="approval-card"
          >
          ...
          <!-- 门卫入厂审核 -->
          <view
            v-if="['门卫入厂审批'].includes(currentNode)"
            class="approval-card"
          >
          ...
          <view
            v-if="['门卫离厂审批'].includes(currentNode)"
            class="approval-card"
          >
