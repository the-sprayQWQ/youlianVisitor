      ...
      {},
      {
        ...
        onSuccess: (res) => {
          ...
          uploadedExcelFile.value = {
            name: res.originalName || 'excel文件.xlsx',
            url: res.message,
            size: res.size,
          }
          // 将文件信息保存到表单数据中
...
<template>
  <view class="container">
    <scroll-view class="content" :class="{ 'has-submit-btn': !applyId || (applyId && isShow) }" scroll-y>
      <form>
        <!-- 申请单号 -->
        ...
        <view class="form-section">
          ...
          <view class="add-employee-btn" @click="addEmployee">
        ...
        <view class="form-section">
          ...
          <view class="excel-upload-container">
            <!-- 上传进度显示 -->
            <view v-if="getExcelUploader().loading.value" class="excel-upload-progress">
              <view class="progress-content">
                <view class="i-carbon-loading rotating text-24px text-blue-500" />
