...
function updateParticipantCards(e: Event) {
  const target = e.target as HTMLInputElement
  const targetCount = Number.parseInt(target.value) || 0
  if (targetCount === '0') {
    ...
    form.value.ylfkApplicationUserList.splice(
      e.target.value,
      form.value.ylfkApplicationUserList.length - e.target.value,
    )
  }
}
