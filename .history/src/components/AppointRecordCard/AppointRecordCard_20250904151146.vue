...
function toDetail(id: string, applyId: string, applicationType: string) {
  emit('toDetail', id, applyId, applicationType)
}
</script>

<template>
  <view v-for="item in props.recordList" :key="item.id" class="item" @click="toDetail(item.id, item.applicationCode, item.applicationType)">
