  ...
  if (processType_dictText === '已通过') {
    ...
    uni.navigateTo({
      url: `/pages/visitorApplication/visitorApplication?id=${id}&applyId=${
        applyId
      }&roleType=${roleType}&applicationType=${applicationType}&currentNode=${currentNode}&view=true`,
    })
  }
  else {
    uni.navigateTo({
      url: `/pages/employApplication/employApplication?applyId=${
        applyId
      }&roleType=${roleType}&applicationType=${applicationType}&currentNode=${currentNode}`,
    })
  }
}
const userStore = useUserStore()
        ...
        <view class="record-list">
          ...
          >
            <view class="record-content">
              <text class="record-title">{{ item.applicationTypeName }}</text>
              <text class="record-info">
                申请单号：{{ item.applicationCode }}
              </text>
              <view class="record-details">
                <text class="record-detail">
                  当前节点：{{ item.currentName }}
                </text>
              </view>
              <text class="record-detail" style="width: 200rpx">
                审批人：{{ item.currentRecordUser }}
              </text>
