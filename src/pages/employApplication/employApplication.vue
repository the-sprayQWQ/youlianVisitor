...
async function getResult(code: string) {
  ...
  if (res.code === 200) {
    ...
    // 根据流程信息设置审批状态回显
    setApprovalStatusFromResult(res.result)
  }
}
    ...
    if (item.applicatiResult_dictText === '通过') {
      ...
      switch (nodeName) {
        case '归口管理部门经理室审核':
          ...
          break
        case '人资部行政部审核':
          // 人资部和行政部都设置为true，因为它们在同一个节点下
          approvals.value.hr = true
          approvals.value.admin = true
          break
        case '门卫入厂审批':
          approvals.value.guardEnter = true
          break
        case '门卫离厂审批':
          approvals.value.guardExit = true
          approvals.value.guardReturnPass = true
          break
        default:
          break
      }
...
const opinionsList = ref([]) // 获取的流程信息
