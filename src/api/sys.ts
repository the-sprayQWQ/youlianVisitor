...
export interface IAppResult {
  result: {
    nodeName: string
    applicatiOpinion: string
    applicatiResult_dictText: string
  }[]
}
/**
 * 获取系统内所有部门
...
export function getApprovalResult(applicationCode: string) {
  return http.get<IAppResult>(
    '/processApplication/processApplication/listProcessNodeRecordByApplicationCode',
    { applicationCode },
  )
}
